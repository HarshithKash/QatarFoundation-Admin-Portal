...
document.getElementById('opportunityForm').addEventListener('submit', function(e) {
    ...
    .then(res => {
        ...
        showToast(editId ? 'Opportunity updated successfully!' : 'Opportunity created successfully!');
...
// ── Render all opportunities from the database ──
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer">
            ...
            <div style="display:flex;gap:8px;">
                <button class="view-course-btn" style="width:auto;padding:8px 16px;" onclick='_viewOpp(${JSON.stringify(opp)})'>View Details</button>
                <button class="action-btn" style="padding:8px;background:var(--surface-2,#f5f5f5);border:none;border-radius:8px;cursor:pointer;" onclick='_editOpp(${JSON.stringify(opp)})' title="Edit"><svg viewBox="0 0 24 24" style="width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2;"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                <button class="action-btn" style="padding:8px;background:var(--surface-2,#f5f5f5);border:none;border-radius:8px;cursor:pointer;color:#e74c3c;" onclick="_deleteOpp(${opp.id})" title="Delete"><svg viewBox="0 0 24 24" style="width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2;"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
            </div>
        </div>`;
    return card;
}
...
function loadOpportunities() {
    fetch('/api/opportunities', { credentials: 'same-origin' })
    .then(r => r.json())
    .then(data => {
        ...
        grid.innerHTML = '';
        if (!data.length) {
            grid.innerHTML = '<p style="color:var(--text-secondary,#888);padding:24px;">No opportunities yet. Click "Add Opportunity" to get started.</p>';
            return;
        }
...
function _editOpp(opp) {
    ...
    document.getElementById('opportunityForm').dataset.editId = opp.id;
    openOpportunityModal();
}

function _deleteOpp(id) {
    if (!confirm('Delete this opportunity permanently?')) return;
    fetch(`/api/opportunities/${id}`, { method: 'DELETE', credentials: 'same-origin' })
    .then(r => r.json())
    .then(res => { if (!res.error) { showToast('Opportunity deleted!'); loadOpportunities(); } })
...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    .then(res => {
        if (res.message === 'Login successful') {
            ...
            generateCaptcha('login');
            setTimeout(() => { showDashboard(res.email); loadOpportunities(); }, 1200);
        } else {
    ...
    .catch(() => { showToast('Server error. Please try again.'); shakeForm('loginForm'); });
});
...
document.getElementById('signupForm').addEventListener('submit', function(e) {
    ...
    .then(r => r.json())
    .then(res => {
        if (res.error) { showToast(res.error); shakeForm('signupForm'); return; }
        showToast('Account created successfully!');
        generateCaptcha('signup');
        this.reset(); checkStrength('');
        setTimeout(() => showPage('loginPage'), 1500);
    })
    .catch(() => { showToast('Server error. Please try again.'); shakeForm('signupForm'); });
});
...
document.getElementById('forgotForm').addEventListener('submit', function(e) {
    ...
    .then(r => r.json())
    .then(res => { showToast(res.message || 'Reset link sent to your email!'); generateCaptcha('forgot'); this.reset(); })
    .catch(() => { showToast('Server error. Please try again.'); });
});
